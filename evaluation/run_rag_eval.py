...
Usage:
  .venv\Scripts\python run_rag_eval.py triad --delay-after-case 5
  .venv\Scripts\python run_rag_eval.py split --output results\split_run.json --delay-after-case 5
"""
...
_NO_INFO_PATTERNS = [
    re.compile(r"không có thông tin", re.IGNORECASE),
    re.compile(r"không có dữ liệu", re.IGNORECASE),
    re.compile(r"không có trong hệ thống", re.IGNORECASE),
    re.compile(r"không tìm thấy", re.IGNORECASE),
    re.compile(r"không có kết quả", re.IGNORECASE),
    re.compile(r"không đủ thông tin", re.IGNORECASE),
]
...
def _is_no_info_case(raw: dict) -> bool:
    text_parts = [
        str(raw.get("input") or ""),
        str(raw.get("actual_output") or ""),
        " ".join(str(x) for x in (raw.get("retrieval_context") or [])),
    ]
    haystack = "\n".join(text_parts)
    return any(pattern.search(haystack) for pattern in _NO_INFO_PATTERNS)
...
def _measure_all(test_case, metrics, delay_between_metrics: float = 0) -> list[dict]:
    out = []
    for idx, m in enumerate(metrics):
        # Nếu không phải metric đầu tiên của testcase này, tiến hành delay để tránh dồn dập Request (Tránh Rate Limit)
        if idx > 0 and delay_between_metrics > 0:
            time.sleep(delay_between_metrics)
        ...
        for m in row.get("metrics") or []:
            ...
            if "leaked" in text.lower():
                ...
                msg = (
                    ...
                    "Hãy tăng thêm thời gian delay bằng: --delay-after-case 6"
                )
                key = "429"
...
def run_triad(
    cases_path: Path, output_path: Path | None, delay_seconds: float = 0
) -> dict:
    ...
    for idx, raw in enumerate(case_list):
            ...
            rows.append(
                {
                    ...
                    "skipped": True,
                    "reason": (
                        "Câu hỏi thuộc nhóm không có thông tin trong hệ thống; "
                        "bỏ qua Answer Relevancy / Faithfulness / Contextual Relevancy "
                        "để không tính nhầm một câu trả lời hợp lệ thành fail."
                    ),
                }
            )
            continue
        ...
        # Chia nhỏ delay giữa các metric bằng một nửa delay_seconds cấu hình
        metric_delay = delay_seconds / 2 if delay_seconds > 0 else 0
        measured = _measure_all(tc, metrics, metric_delay)
...
def run_split(
    ...
    def _maybe_sleep() -> None:
        nonlocal case_index
        case_index += 1
        if case_index > 1 and delay_seconds > 0:
            time.sleep(delay_seconds)
    ...
    for raw in ret_cases:
        ...
        if not raw.get("expected_output"):
            ...
            continue
        tc = _case_to_llm_test_case(raw)
        measured = _measure_all(tc, retrieval_metrics, metric_delay)
            ...
            rows.append(
                {
                    ...
                    "reason": (
                        "Câu hỏi thuộc nhóm không có thông tin trong hệ thống; "
                        "bỏ qua metric answer/faithfulness để tránh chấm sai."
                    ),
                }
            )
            continue
        tc = _case_to_llm_test_case(raw)
        measured = _measure_all(tc, generation_metrics, metric_delay)
...
def main() -> None:
    ...
    common.add_argument(
        "--delay-after-case",
        type=float,
        # Tăng mặc định lên 5-6 giây để an toàn hơn cho gói Free tier của Gemini
        default=float(os.getenv("EVAL_DELAY_AFTER_CASE_SECONDS", "5.0")),
        metavar="SEC",
        help="Sleep SEC seconds between test cases (Free tier Gemini ~15 RPM; try 5-6). "
    ...
    delay = float(getattr(args, "delay_after_case", 5.0) or 5.0)
